...
- **TypeScript prop inference.** Props are inferred statically from the component's TypeScript types at startup via `react-docgen-typescript` or equivalent; re-analysed on file change. The current implementation analyses the shim's exported wrapper; the correct target is the shim's interior — the components the shim renders. Tracked on AWK-16; reword the issue's AC before kickoff.
