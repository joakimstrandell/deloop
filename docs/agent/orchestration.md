...
How CPTO orchestrates Implementer and Reviewer subagents. For the higher-level process (issue → branch → PR → merge, worktree/sequential/autonomous mode/recovery), see [workflow.md](workflow.md).

## Spawning

- **Fresh agents per issue.** New Implementer and Reviewer per issue. Never reuse across issues. Reusable knowledge → playbooks, memory, or the issue tracker; never agent context.
...
- **Max 2 review cycles.** After cycle 2, CPTO arbitrates remaining items and locks scope. Out-of-scope items become new issues.
